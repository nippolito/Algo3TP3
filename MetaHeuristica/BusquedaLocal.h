...
int BusquedaLocal(set<int> matrix [], set<int>& solAct, int n , int m){
	...
	while(noEncontrePozo){
		...
		for(int i = 0; i < n; i ++){
			if(solAct.find(i) == solAct.end()){   // si el nodo no pertenece en la actual solución me fijo si es vecino de todo los que si lo hacen
