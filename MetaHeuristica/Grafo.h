...
struct GraphTest			    // n-> Number of vertices, m-> Number of edges
{
    int n, m;
    vector<vector<int> > listaAdy;		   // graph se representa con lista de adyacencia
};

void createGraph(GraphTest* graph, int n, int m){
...
void addEdge(struct GraphTest* grafo, int src, int dest){
...
void mostrarGrafo(GraphTest* grafo){
...
int gradoNodo(struct GraphTest* grafo, int nodo){
	return grafo->listaAdy[nodo].size();
}

int nodoGradoMaximo(struct GraphTest* grafo){
	int res = 0;
...
bool esVecino(struct GraphTest* grafo, int nodo1, int nodo2){
	int z1,z2, n;
...
bool nodoFormaClique(struct GraphTest* grafo, vector<int> vec, int nodo, int cantNodosClique){
	int k = 0;
...
bool noEstabaEnClique(struct GraphTest* grafo, vector<int> vec, int nodo){
	if(vec[nodo] == 0){
		return true;
	}else{
		return false;
...
int calcFrontera(struct GraphTest* grafo, vector<int> & solAct){
...
bool nodoFormaCliqueA(struct GraphTest* grafo, vector<int>& vec, int nodo){
	bool res = true;
